...
if (query != null){
    ...
    if (query.get("left").equalsIgnoreCase("__UID__") && objectClass.equalsIgnoreCase("__ACCOUNT__")) query.put("left","u.id");
...
switch ( objectClass ) {
    ...
    sql.eachRow("SELECT * FROM usr_user as u LEFT JOIN usr_user_role as ur on u.id=ur.user_id LEFT JOIN role as r on ur.role_id=r.id" + where, {result.add([__UID__:it.id, __NAME__:it.username, full_name:it.full_name, table_rows:it.table_rows, role:it.name])} );
    break
